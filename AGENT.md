...
| Notes | `GET, POST /v1/notes`; `GET, PATCH, DELETE /v1/notes/{noteId}` | List/search notes with bounded summaries; fetch a full Markdown body only for a selected note; create, edit, and delete. Supports Google Calendar style reminders with `reminder_at` timestamp and pre-alert `reminder_offsets`. |
